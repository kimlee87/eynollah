...
class sbb_predict:
    ...
    def otsu_copy(self, img):
        ...
        _, threshold3 = cv2.threshold(
            img[:, :, 2], 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )
    ...
    def soft_dice_loss(self, y_true, y_pred, epsilon=1e-6):
        ...
        numerator = 2.0 * K.sum(y_pred * y_true, axes)
    ...
    def weighted_categorical_crossentropy(self, weights=None):
        ...
        def loss(y_true, y_pred):
            ...
            per_pixel_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=labels_floats, logits=y_pred
            )
    ...
    def start_new_session_and_model(self):
        ...
        if self.task != "classification" and self.task != "reading_order":
            self.img_height = self.model.layers[
                len(self.model.layers) - 1
            ].output_shape[1]
            self.img_width = self.model.layers[len(self.model.layers) - 1].output_shape[
                2
            ]
            self.n_classes = self.model.layers[len(self.model.layers) - 1].output_shape[
                3
            ]
    ...
    def visualize_model_output(self, prediction, img, task):
        if task == "binarization":
            ...
            rgb_colors = {
                ...
                "2": [60, 76, 231],  # text
                "3": [219, 152, 52],  # image
                "4": [182, 89, 155],  # separator in case of no headings
                # "4": [34, 126, 230],  # heading
                # "5": [182, 89, 155],  # separator
                # "6": [125, 125, 0],
                # "7": [0, 125, 255],
                # "8": [0, 125, 0],
                # "9": [125, 125, 125],
            ...
            for unq_class in unique_classes:
                ...
                layout_only[:, :, 0][prediction[:, :, 0] == unq_class] = (
                    rgb_class_unique[0]
                )
                layout_only[:, :, 1][prediction[:, :, 0] == unq_class] = (
                    rgb_class_unique[1]
                )
                layout_only[:, :, 2][prediction[:, :, 0] == unq_class] = (
                    rgb_class_unique[2]
                )
            ...
            added_image = cv2.addWeighted(
                img, 0.5, layout_only, 0.5, 0
            )  # instead of 0.5 and 0.1
    ...
    def predict(self, image_dir):
        if self.task == "classification":
            ...
            img_1ch = cv2.resize(
                img_1ch,
                (
                    self.config_params_model["input_height"],
            ...
            for j in range(len(cy_main)):
                img_header_and_sep[
                    int(y_max_main[j]) : int(y_max_main[j]) + 12,
                    int(x_min_main[j]) : int(x_max_main[j]),
                ] = 1
            ...
            for i in range(len(co_text_all)):
                ...
                img_label = cv2.fillPoly(
                    img_label, pts=[co_text_all[i]], color=(1, 1, 1)
                )
            ...
            while index_update >= 0:
                ...
                for j in ij_list:
                    ...
                    input_1[batch_counter, :, :, 0] = img1[:, :, 0] / 3.0
                    input_1[batch_counter, :, :, 2] = img2[:, :, 0] / 3.0
                    input_1[batch_counter, :, :, 1] = img3[:, :, 0] / 5.0
                    ...
                    if batch_counter == inference_bs or (
                        (tot_counter // inference_bs) == full_bs_ite
                        and tot_counter % inference_bs == last_bs
                    ):
                ...
                starting_list_of_regions, index_update = (
                    update_list_and_return_first_with_length_bigger_than_one(
            ...
            tree_xml.write(
                os.path.join(self.out, file_name + ".xml"),
            ...
            if self.patches:
                ...
                for i in range(nxf):
                    for j in range(nyf):
                        ...
                        if i == 0 and j == 0:
                            seg = seg[
                                0 : seg.shape[0] - margin, 0 : seg.shape[1] - margin
                            ]
                            prediction_true[
                            ...
                            seg = seg[
                                margin : seg.shape[0] - 0, margin : seg.shape[1] - 0
                            ]
                            prediction_true[
                            ...
                            seg = seg[
                                margin : seg.shape[0] - 0, 0 : seg.shape[1] - margin
                            ]
                            prediction_true[
                                index_y_d + margin : index_y_u - 0,
                                index_x_d + 0 : index_x_u - margin,
                            ...
                            seg = seg[
                                0 : seg.shape[0] - margin, margin : seg.shape[1] - 0
                            ]
                            prediction_true[
                                index_y_d + 0 : index_y_u - margin,
                                index_x_d + margin : index_x_u - 0,
                ...
                prediction_true = cv2.resize(
                    prediction_true,
                    (self.img_org.shape[1], self.img_org.shape[0]),
                    interpolation=cv2.INTER_NEAREST,
                )
    ...
    def run(self):
        ...
        if self.image:
            ...
            if self.task == "classification" or self.task == "reading_order":
                ...
                img_seg_overlayed, only_layout = self.visualize_model_output(
                    res, self.img_org, self.task
                )
                if self.save:
...
def main(
    ...
    if task != "classification" and task != "reading_order":
        if image and not save:
            print(
                "Error: You used one of segmentation or binarization task with image input but not set -s, you need a filename to save visualized output with -s"
            )
            sys.exit(1)
        if dir_in and not out:
            print(
                "Error: You used one of segmentation or binarization task with dir_in but not set -out"
            )
            sys.exit(1)
    x = sbb_predict(
        image,
